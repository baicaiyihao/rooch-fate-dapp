...
//     const [ç, setRaffleConfig] = useState<any>(null);
...
import { useState, useEffect } from "react";
import { useCurrentAddress } from "@roochnetwork/rooch-sdk-kit";
import { Box, Card, Button, Divider, Fade, Grid, Stack, Typography, Chip, CardContent } from "@mui/material";
import { LoadingButton } from "@mui/lab";
import { motion } from "framer-motion";
import Confetti from "react-confetti";
import { useWindowSize } from "react-use";
...
import {backgroundAnimation, AnimatedBackground} from "../components/shared/animation_components";
...
function RafflePage() {
  ...
  const { GetCheckInRaffleByFate, ClaimMaxRaffle, QueryCheckInRaffle, QueryCheckInRaffleRecord } = Raffle();
  ...
  const fetchData = async () => {
    ...
    setErrorMessage(null);
    try {
      const [checkInData, raffleInfoData, raffleRecordData] = await Promise.all([
        QueryCheckInRecord(),
        QueryCheckInRaffle(),
        QueryCheckInRaffleRecord()
      ]);
      
      setCheckInRecord(checkInData);
      setRaffleInfo(raffleInfoData);
      setRaffleRecord(raffleRecordData);
    } catch (error) {
      console.error("获取数据失败:", error);
      setErrorMessage("获取数据失败，请稍后再试");
    } finally {
      setIsDataLoading(false);
    }
  };
  ...
  const handleWeekRaffle = async () => {
    ...
    try {
      await GetWeekRaffle();
      await fetchData();
      setSuccessMessage('每周抽奖成功！');
      setShowSuccess(true);
      setShowConfetti(true);
      setTimeout(() => setShowSuccess(false), 3000);
    } catch (error) {
      console.error("每周抽奖失败:", error);
      setErrorMessage("每周抽奖失败，请稍后再试");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFateRaffle = async () => {
    ...
    try {
      ...
      await fetchData();
      setSuccessMessage('FATE抽奖成功！');
      setShowSuccess(true);
      setShowConfetti(true);
      setTimeout(() => setShowSuccess(false), 3000);
    } catch (error) {
      console.error("FATE抽奖失败:", error);
      setErrorMessage("FATE抽奖失败，请确保有足够的FATE代币");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      >
        <Stack direction="row" justifyContent="space-between" alignItems="center" className="mb-8">
          ...
          <Typography variant="h4" className="font-bold">抽奖系统</Typography>
          <Box width={100} />
        </Stack>

        {errorMessage && (
          <Box sx={{ p: 2, mb: 3, bgcolor: '#FFEBEE', borderRadius: 2, border: '1px solid #FFCDD2' }}>
            <Typography color="error" sx={{ display: 'flex', alignItems: 'center' }}>
              <Box component="span" sx={{ mr: 1, fontSize: '1.2rem' }}>⚠️</Box>
              {errorMessage}
            </Typography>
          ...
          <Grid item xs={12} md={6}>
            <StyledCard elevation={3} className="mb-8">
              <CardContent>
                ...
                {isDataLoading ? (
                  <Stack spacing={2}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography>每周抽奖次数:</Typography>
                      <Box width={80} height={32}><motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} style={{ width: '100%', height: '100%', backgroundColor: '#f0f0f0', borderRadius: '16px' }} /></Box>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography>FATE抽奖次数:</Typography>
                      <Box width={80} height={32}><motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} style={{ width: '100%', height: '100%', backgroundColor: '#f0f0f0', borderRadius: '16px' }} /></Box>
                    </Box>
                  </Stack>
                ) : (
                  <Stack spacing={2}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography>每周抽奖次数:</Typography>
                      <Fade in={true}>
                        <Chip label={checkInRecord?.lottery_count || 0} color="primary" sx={{ fontWeight: 'bold' }} />
                      </Fade>
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography>FATE抽奖次数:</Typography>
                      <Fade in={true}>
                        <Chip label={raffleRecord?.raffle_count || 0} color="primary" sx={{ fontWeight: 'bold' }} />
                      </Fade>
                    </Box>
                  </Stack>
                )}
            ...
            <Stack direction="row" spacing={2} justifyContent="center" className="mt-4">
              <StyledButton
                ...
                startIcon={<span>📅</span>}
                disabled={isDataLoading || parseInt(checkInRecord?.lottery_count || '0') === 0}
              >
                每周抽奖
              </StyledButton>
              <StyledButton
              ...
              <StyledButton
                ...
                startIcon={<span>🏆</span>}
                disabled={isDataLoading || parseInt(raffleRecord?.raffle_count || '0') < 10}
              >
                领取保底奖励
              </StyledButton>
            </Stack>
              ...
              <CardContent>
                ...
                {isDataLoading ? (
                  <Stack spacing={2}>
                    {[1, 2, 3].map((item) => (
                      <Box key={item} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Box width={120} height={24}><motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} style={{ width: '100%', height: '100%', backgroundColor: '#f0f0f0', borderRadius: '4px' }} /></Box>
                        <Box width={80} height={24}><motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.5 }} style={{ width: '100%', height: '100%', backgroundColor: '#f0f0f0', borderRadius: '4px' }} /></Box>
                      </Box>
                    ))}
                  </Stack>
                ) : raffleInfo ? (
                  <Stack spacing={2}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography>抽奖消耗FATE:</Typography>
                      <Chip label={`${raffleInfo.fate_cost || 0}`} color="secondary" sx={{ fontWeight: 'bold' }} />
                    </Box>
                    <Divider />
                    <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mt: 1 }}>奖池信息:</Typography>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1, bgcolor: '#FFF9C4', borderRadius: 1 }}>
                      <Typography>特等奖概率:</Typography>
                      <Chip label={`${raffleInfo.grand_prize_weight || 0}%`} color="warning" sx={{ fontWeight: 'bold' }} />
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1, bgcolor: '#E1F5FE', borderRadius: 1 }}>
                      <Typography>二等奖概率:</Typography>
                      <Chip label={`${raffleInfo.second_prize_weight || 0}%`} color="info" sx={{ fontWeight: 'bold' }} />
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 1, bgcolor: '#F1F8E9', borderRadius: 1 }}>
                      <Typography>三等奖概率:</Typography>
                      <Chip label={`${raffleInfo.third_prize_weight || 0}%`} color="success" sx={{ fontWeight: 'bold' }} />
                    </Box>
                  </Stack>
                ) : (
                  <Typography color="text.secondary">暂无配置信息</Typography>
        ...
        <StyledCard elevation={2} className="mt-8">
          <CardContent>
            ...
            <Typography paragraph>1. 每周抽奖基于签到记录，每次消耗1次抽奖机会。</Typography>
            <Typography paragraph>2. FATE抽奖使用FATE代币参与，每次抽奖将消耗一定数量的FATE。</Typography>
            <Typography paragraph>3. 抽奖次数达到10次后可领取保底奖励。</Typography>
            <Typography paragraph>4. 所有抽奖均有机会获得特等奖、二等奖或三等奖，概率如奖池信息所示。</Typography>
          </CardContent>
        </StyledCard>
      </Stack>
    </>
